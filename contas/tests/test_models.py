...
class TestHistoricoConta(TestCase):
    def test_historico_conta_create(self):
        contato = Contato.objects.create(nome='fernando',
                                         email='fernando@email.com',
                                         telefone='016999999999')

        conta = Conta.objects.create(contato=contato,
                                     valor=200.50,
                                     data_recebimento_esperado=datetime.now().date(),
                                     data_recebimento=datetime.now().date())

        historico = HistoricoConta.objects.create(conta=conta,
                                                  valor=conta.valor,
        ...
        self.assertEqual(conta.data_recebimento_esperado,
                         historico.data_recebimento_esperado)
        self.assertEqual(conta.data_recebimento,
                         historico.data_recebimento)
