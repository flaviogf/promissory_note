...
class ContasContatoView(View):
    ...
    def get(self, request, contato_id):
        ...
        contas_recebidas = [
            it
            for it in Conta.objects.filter(contato=contato).order_by(
                "-data_recebimento"
            )
            if it.recebida
        ]
        contas_pendentes = [
            it for it in Conta.objects.filter(contato=contato) if not it.recebida
        ]

        conta_form = ContaForm(initial={"contato": contato.contato_id})
        return render(
            request,
            "contas/contato/create.html",
            {
                "contato": contato,
    ...
    def post(self, request, contato_id):
        ...
        if conta_form.is_valid():
            conta_form.save()
            url = reverse("contas:contato-create", kwargs={"contato_id": contato_id})
            return redirect(url)
